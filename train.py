...
def get_cmd():
    ...
    parser.add_argument("-m", "--model", default="CLHE",
    ...
    parser.add_argument("--seed", default=2306, type=int, help="")
    ...
    parser.add_argument("--custom_log_path", default="", type=str, help="custom log path for tuning log")
    parser.add_argument("--iui_path", default="", type=str, help="iui path for gatv2")
    
    parser.add_argument("--bundle_alpha", default=1.0, type=float, help="hyperparam for fusion")
    parser.add_argument("--item_alpha", default=1.0, type=float, help="hyperparam for fusion")
    parser.add_argument("--alpha_residual", default=1.0, type=float, help="hyperparam for fusion")
    parser.add_argument("--hyper_num", default=4, type=int, help="numbers of hyperedges for hypergraph")
    parser.add_argument("--n_layer_gat", default=1, type=int, help="numbers of layers for gatv2")
    parser.add_argument("--alpha_bundle_loss", default=0.05, type=float, help="hyperparam for contrastive bundle loss")
    parser.add_argument("--alpha_item_loss", default=0.1, type=float, help="hyperparam for contrastive item loss")
    parser.add_argument("--residual_type", default=0, type=int, help="type of residual connection")
...
def main():
    ...
    if conf["custom_log_path"] != "":
        log_path = log_path + "/" + conf["custom_log_path"] + ".txt"
    else:
...
def write_log(conf, run, log_path, topk, step, metrics):
    ...
    if conf["wandb"]:
        wandb.log({
            f"test recall@{topk}": test_scores["recall"][topk],
            f"test ndcg@{topk}": test_scores["ndcg"][topk],
            f"val ndcg@{topk}": val_scores["ndcg"][topk],
            f"val recall@{topk}": val_scores["recall"][topk]
        })
...
def log_metrics(conf, model, metrics, run, log_path, checkpoint_model_path, checkpoint_conf_path, epoch, batch_anchor, best_metrics, best_perform, best_epoch):
    for topk in conf["topk"]:
        write_log(conf, run, log_path, topk, batch_anchor, metrics)
