    ...
    def get_features(self):
        try:
            content_feature = torch.load(os.path.join(
                self.path, self.name, 'content_feature.pt'), map_location=self.device, weights_only=True)
            if not self.is_openai_embedding:
                description_feature = torch.load(os.path.join(
                    self.path, self.name, 'description_feature.pt'), map_location=self.device, weights_only=True)
            else:
                description_feature = torch.load(os.path.join(
                    self.path, self.name, 'openai_description_feature.pt'), map_location=self.device, weights_only=True)
        except:
        ...
        cf_feature = torch.load(os.path.join(
            self.path, self.name, 'item_cf_feature.pt'), map_location=self.device, weights_only=True)
